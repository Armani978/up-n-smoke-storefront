...
type ProductForm = { name: string; sku: string; description: string; price: string; status: string };
const empty: ProductForm = { name: "", sku: "", description: "", price: "", status: "published" };
...
export function InventoryManager({ initialProducts, canWrite }: { initialProducts: AdminProduct[]; canWrite: boolean }) {
  ...
  const openEdit = (product: AdminProduct) => { setEditing(product); setForm({ name: product.name, sku: product.sku, description: product.description, price: String(product.price), status: product.status }); };
