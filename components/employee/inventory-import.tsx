...
type ImportResult = { dry_run: boolean; total_rows: number; matched_count: number; created_count: number; repaired_count: number; level_count: number; storefront_count: number; hidden_count: number; message?: string };
...
export function InventoryImport() {
  ...
  return <section className="import-console"><header><div><FileSpreadsheet /><span><small>Daily catalog operation</small><h2>CLOVER XLSX → MEDUSA</h2></span></div><a href="/UP-N-SMOKE-Medusa-Inventory-Template.xlsx" download>Download template ↓</a></header><p>Preview the complete workbook before applying product, title, inventory, and storefront visibility changes. “Show in register app” values are carried through to the storefront; you can fine-tune any item later in Inventory.</p><label className="file-drop"><UploadCloud /><b>{file?.name ?? "DROP OR SELECT AN XLSX WORKBOOK"}</b><span>Maximum 10 MB · no changes happen during preview</span><input type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(event) => { setFile(event.target.files?.[0] ?? null); setResult(null); }} /></label><div className="import-actions"><Button onClick={() => void upload(true)} disabled={!file || busy}>{busy ? "Reading…" : "Preview import"}</Button>{result?.dry_run && changes > 0 && <Button variant="dark" onClick={() => void upload(false)} disabled={busy}><FileCheck2 /> Apply {changes} changes</Button>}</div>{result && <div className={result.message ? "import-result error" : "import-result"}>{result.message ? <b>{result.message}</b> : <><b>{result.dry_run ? "PREVIEW READY" : "MEDUSA UPDATED"}</b><span>{result.total_rows} rows</span><span>{result.matched_count} stock matches</span><span>{result.created_count} new products</span><span>{result.storefront_count} on storefront</span><span>{result.hidden_count} hidden</span></>}</div>}</section>;
}
